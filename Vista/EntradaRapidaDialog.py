...
class EntradaRapidaDialog(QDialog):
    def __init__(self, parent, p, ctrl):
        super().__init__(parent)
        self.ctrl = ctrl
        self.producto = p
        self.setWindowTitle("Entrada Rapida de Mercancia")
        self.setModal(True)
        self.construir()
    ...
    def construir(self):
        ...
        tit = QLabel("INGRESO DIRECTO", panel)
        ...
        lblCant = QLabel("CANTIDAD A INGRESAR:", panel)
        ...
        btnGuardar = Estilos.botonPrimario("Ingresar")
    ...
    def guardar(self):
        try:
            ...
            self.ctrl.getApp().registrarRetiro(f"ingreso directo {self.producto.getNombre()}", cantidad * self.producto.getPrecio())
            
            QMessageBox.information(self, "Exito", "Inventario actualizado exitosamente")
            self.accept()
        except ValueError:
