...
public class NuevaOrdenDialog extends JDialog {
	...
	private void construir() {
		...
		header.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(0, 0, 1, 0, Estilos.BORDE),
		...
		txtBusquedaProd.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createLineBorder(Estilos.BORDE, 2),
		...
		String[] cols = { "Producto", "Cant.", "Costo Unit.", "Subtotal", "Quitar" };
		...
		tablaItems.getColumnModel().getColumn(1).setMaxWidth(70);
		tablaItems.getColumnModel().getColumn(2).setMaxWidth(110);
		tablaItems.getColumnModel().getColumn(3).setMaxWidth(110);
		tablaItems.getColumnModel().getColumn(4).setMaxWidth(90);

		tablaItems.getColumnModel().getColumn(4).setCellRenderer(new VentaPanel.BtnRenderer());
		tablaItems.getColumnModel().getColumn(4).setCellEditor(new VentaPanel.BtnEditor(idx -> {
			try {
		...
		footer.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createMatteBorder(1, 0, 0, 0, Estilos.BORDE),
	...
	private void buscarYAgregar() {
		...
		for (Producto p : ctrl.getProductos()) {
			if (p.getNombre().toLowerCase().contains(q.toLowerCase()))
				res.add(p);
			if (res.size() >= 5)
				break;
		}
		if (res.isEmpty()) {
			JOptionPane.showMessageDialog(this, "No se encontraron productos.", "Aviso",
		...
		for (OrdenCompra.ItemOrden item : items) {
			if (item.getProducto().getId().equals(sel.getId())) {
				item.setCantidadSolicitada(item.getCantidadSolicitada() + 1);
				actualizarTablaItems();
				txtBusquedaProd.setText("");
				return;
			}
		}
		items.add(new OrdenCompra.ItemOrden(sel, 1, sel.getPrecio() * 0.75));
		actualizarTablaItems();
		txtBusquedaProd.setText("");
	}
	...
	private void actualizarTablaItems() {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			modeloItems.addRow(new Object[] { item.getProducto().getNombre().toUpperCase(),
					String.format("%.0f", item.getCantidadSolicitada()), String.format("$%.2f", item.getPrecioCosto()),
					String.format("$%.2f", item.getSubtotal()), String.valueOf(i) });
	...
	private void confirmarOrden() {
		...
		OrdenCompra.TipoPago tipoPago = "CREDITO".equals(cmbTipoPago.getSelectedItem()) ? OrdenCompra.TipoPago.CREDITO
				: OrdenCompra.TipoPago.CONTADO;
		...
		JOptionPane.showMessageDialog(this, "Orden " + orden.getFolioCorto() + " generada correctamente.", "Exito",
				JOptionPane.INFORMATION_MESSAGE);
		dispose();
	}
