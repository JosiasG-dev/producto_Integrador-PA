...
public class NuevaOrdenDialog extends JDialog {
	...
	private void actualizarTablaItems() {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			String cantStr = p.esPorPieza() ? String.format("%.0f", item.getCantidadSolicitada())
					: String.format("%.2f", item.getCantidadSolicitada());
			modeloItems.addRow(new Object[] { p.getNombre().toUpperCase(), p.getUnidad(), cantStr,
					String.format("$%.2f", item.getPrecioCosto()), String.format("$%.2f", item.getSubtotal()),
					String.valueOf(i) });
