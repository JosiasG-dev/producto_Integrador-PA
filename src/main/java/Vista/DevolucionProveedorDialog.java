...
public class DevolucionProveedorDialog extends JDialog {

	private final InventarioControlador ctrl;
	private final Producto producto;

	public DevolucionProveedorDialog(Window parent, Producto p, InventarioControlador ctrl) {
		super(parent, "Devolucion a Proveedor", ModalityType.APPLICATION_MODAL);
		this.ctrl = ctrl;
		this.producto = p;
		construir();
	}
	...
	private void construir() {
		...
		JLabel tit = new JLabel("DEVOLVER PRODUCTOS");
		...
		JLabel lblCant = new JLabel("CANTIDAD A DEVOLVER:");
		...
		JButton btnGuardar = Estilos.botonPeligro("Devolver");
		...
		btnGuardar.addActionListener(e -> {
			try {
				...
				if (cantidad > producto.getStock()) {
					JOptionPane.showMessageDialog(this, "No puede devolver mas del stock existente", "Aviso", JOptionPane.WARNING_MESSAGE);
					return;
				}
				producto.setStock(producto.getStock() - cantidad);
				ctrl.guardarProducto(producto);
				JOptionPane.showMessageDialog(this, "Devolucion procesada exitosamente");
				dispose();
